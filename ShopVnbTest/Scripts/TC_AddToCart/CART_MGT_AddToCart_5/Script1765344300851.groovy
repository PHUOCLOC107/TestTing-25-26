...
//TC5 – Add product exceeding stock
WebUI.openBrowser('')
...
for (int i = 1; i <= 2; i++) {
	WebUI.click(findTestObject('Product Detail/Btn_AddQty'))
}
...
if (WebUI.waitForAlert(2, FailureHandling.OPTIONAL)) {
	...
	println("Alert xuất hiện: " + alertText)
	assert alertText.contains("Sản phẩm này chỉ còn lại")
	WebUI.acceptAlert()
} else {
	KeywordUtil.markFailed("❌ Không xuất hiện alert hết hàng!")
}
WebUI.closeBrowser()